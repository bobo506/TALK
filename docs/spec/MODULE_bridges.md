...
`scripts/talk_workflow.py` 提供离线 `validate` / `summary`，默认摘要上限 1200 字符，显式标注截断；`--expect-task-id` 可核对本次报告任务号。完整使用合同见 `docs/guides/TASK_WORKFLOW.md`。它拒绝字段/类型/长度不合规、完成状态与未完成或失败验证矛盾的交付，深嵌套输入返回短错误；校验不代表业务验收，任务号匹配不证明事实归属。该脚本不联网、不自动收取任务，不改变现有 bridge、MCP、服务端和任务状态语义，也未接入自动强制执行。
...
摘要文本上限 1200 字符，正常 JSON 响应上限 6000 字符；阻塞/未完成优先，数量与省略信息显式展示。detail 按结果消息 ID、offset/limit 分页（默认 2000、最多 4000 字符），可选择结构化字段。调用者需携带 expect_sha256 核对同一消息的内容版本；结果 ID 或所提供 hash 不符则 stale_reference 且不返回正文，应重新开始。分页可以重建服务端已存原文，不能恢复上游截断的内容。完整合同及结构化结果提交方式见 TASK_WORKFLOW.md。

DeepSeek #42/#44 实现返修、Kimi #43/#45 独立复核；最终 140 项定向测试与 13 项专项断言通过，真实匿名管道 stdio 0 次退化。普通终端 MCP 暴露新工具；pi TypeScript 扩展未改。当前宿主目录尚未加载，须重连后做一次真实入口冒烟，不将隔离 stdio 验证称为宿主端到端验收。
