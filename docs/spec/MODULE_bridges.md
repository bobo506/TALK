...
`scripts/talk_workflow.py` 提供离线 `validate` / `summary`，自 2026-09-13 起合法结构化摘要默认完整输出；可显式指定 `--max-chars` 限长并标注截断；`--expect-task-id` 可核对本次报告任务号。完整使用合同见 `docs/guides/TASK_WORKFLOW.md`。它拒绝字段/类型/长度不合规、完成状态与未完成或失败验证矛盾的交付，深嵌套输入返回短错误；校验不代表业务验收，任务号匹配不证明事实归属。该脚本不联网、不自动收取任务，不改变现有 bridge、MCP、服务端和任务状态语义，也未接入自动强制执行。


### MCP 只读交付摘要与补读（2026-09-12）

新增 `talk_get_delivery`，既有工具默认合同不变。默认 summary 区分 runner、工作流与结构化交付自报；只有 schema 校验和实际 task.id 匹配才采用业务结论，旧文本 unknown、错号或不合法自报 invalid，不自动 collect/accept。源为服务端结果消息，按结果 ID 和 Hall 作用域精确查询，不按报告路径读本机文件。

2026-09-13 更新：合法结构化摘要默认不设文本/JSON 总长度上限，也不按条目长度机械裁剪；原 1200/6000 默认值已取消。三类完成状态正文完整放在 `preview.*.items`，`summary_text` 给出结论/计数/定位，并完整承载验证证据、限制、下一步及基线/文件等必要信息；完整性按同次响应合看，正文不重复。输入 schema 与 64 KiB 拒收保护保持。仅显式限长或旧自由文本预览可能省略，必须结合标志及补读处理。detail 按结果消息 ID、offset/limit 分页（默认 2000、最多 4000 字符），可选择结构化字段。调用者需携带 expect_sha256 核对同一消息的内容版本；结果 ID 或所提供 hash 不符则 stale_reference 且不返回正文，应重新开始。分页可以重建服务端已存原文，不能恢复上游截断的内容。完整合同及结构化结果提交方式见 TASK_WORKFLOW.md。

DeepSeek #42/#44 实现返修、Kimi #43/#45 独立复核；最终 140 项定向测试与 13 项专项断言通过，真实匿名管道 stdio 0 次退化。普通终端 MCP 暴露新工具；pi TypeScript 扩展未改。2026-09-13 重连后旧版真实入口冒烟已通过：#45 原文两页 773 字符与整页一致、hash 一致、收取状态不变；该证据不覆盖其后修改的默认完整摘要合同，新版需再次重连后补测。
