...
- `runtime=dsh` 的预检会自动追加项目内 `.talk/dsh/preflight-ephemeral.cordis.yml`，关闭 DSH 会话持久化与 checkpoint policy；正常判断和同轮一次协议修复都不生成 DSH 会话。DeepSeek 跨轮询默认只尝试 1 轮，其他 runtime 默认最多 3 轮，可由 `--task-preflight-max-attempts 1..3` 显式覆盖。
...
- 委派弹窗使用有背景、边框和阴影的实体卡片，并按两阶段上下文切换标题、说明、Agent 标签与提交按钮：根入口显示“根任务负责人”，根详情入口显示“子任务执行 Agent”；两种模式的表单标题和下拉框可访问名称与当前上下文一致。
...
- async / sync client 与 Codex MCP / pi extension 已覆盖项目化创建、任务类型 / Review 策略 / 关系字段、单任务及关系读取、结构化门禁结论、协作状态过滤、澄清、接受、等待、Hall 回复、安全取消和结果收取。
- bundled runner 已把新任务结果写入对应 Task Hall，但仍兼容无 `hall_group_id` 的旧任务全局回传。
- `talk_wait_tasks` 当前是最长 30 秒的客户端轮询，不是服务端事件流；项目 Agent 发现现已返回业务角色、决策分级、能力摘要、实例与聚合可用状态。
...
- [x] 根任务与子任务创建弹窗使用实体卡片，并清楚区分“根任务负责人”和“子任务执行 Agent”两阶段分配关系及可访问名称。
