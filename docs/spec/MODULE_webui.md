...
- 每条任务meta行新增“耗时 HH:MM:SS”，复用已有 claimed_at→finished_at，不含created_at前后的排队或result_collected_at等待收取时间；重试按最近一次claim，不累计历史尝试。
- in_progress、claimed_at有效且finished_at为空时，每秒按当前时刻重算；终态使用finished_at固定值。缺失/无效/负时间差显示—；无结束时间的澄清/人工决策状态不继续走秒。小时超过24/99仍累计显示。
- 单个timer挂载到startChat，重复启动先清理，登出清理；只更新运行中耗时节点的textContent，不增网络请求、不重绘列表。状态沿用5秒轮询，结束后最多约5秒才固定。
- UTC无时区/空格分隔、显式偏移/Z的时间解析已在Node/V8验证；后端6位微秒依赖引擎宽松解析，未验证Safari/WebKit。静态资源版本20260913-task-duration。
- Kimi #54开发、DeepSeek #55独立复核PASS，48项定向回归、73条时间样本交叉比对及11种状态门控检查通过；无真实浏览器视觉/HTTP端到端验证，列表紧凑度与换行由用户验收。纯前端，无API/DB变更。

## 2026-09-18 REQ-2 项目开发要求编辑区

- 当前项目“角色”页右侧详情顶部新增项目级编辑区，无角色也可用；human可保存，agent只读。复用项目详情GET与单字段PATCH，不改变任务创建接口。
- 仅保存最新文本；空白清空，非空白原文保留，20000 Unicode码点，判空与Python集合一致。严格核实保存响应同项目/字段存在/值一致；旧后端缺字段显示不支持并禁用保存。
- 草稿按账号+项目留在内存，同项目切角色/任务/群聊及轮询不覆盖输入，跨项目往返可恢复；刷新或登出丢失未保存草稿。请求序号隔离迟到响应，保存中继续编辑保留新草稿。
- 可见性在app.js的renderTaskDetailsPanel统一同步，群聊/任务页隐藏。保存/放弃修改/重试、字数、未保存/错误状态明确，不自动写入占位示例。
- 资源版本20260918-req2-rework。Kimi #94/#96开发返工、DeepSeek #95/#97独立复核，最终Node78、Python49+95通过；真实页面未测。详见 [验收指南](../guides/PROJECT_REQUIREMENTS_ACCEPTANCE.md)。

## 2026-09-18 ROLE-1 角色说明与参与任务执行耗时

- lead保持“统筹任务”；dev/developer/reviewer统一“执行工作”，说明为“负责执行分配的工作、提交完成结果，并交叉验证其他角色的成果。”。列表与详情复用展示映射，仅显示变化，不改后台职责、权限或任务类型。
- 角色参与任务表为任务/执行耗时/任务状态/操作，原参与关系、筛选和查看保留；耗时复用claimed_at→finished_at及已有单timer，运行中更新textContent、终态固定、无效显示—，不含排队/收取。任务状态5秒轮询语义不变。
- 资源版本20260918-role-1。Kimi #98开发、DeepSeek #99独立复核通过：Node84、Python5及独立探针9项通过；2026-09-18用户确认本片页面验收通过，未单独声明窄屏尺寸覆盖。自由文本角色命中JS原型键的显示问题登记非阻塞待办。


## 2026-09-20 C1b-S2：角色页唯一项目主控（基本验收通过，布局后由ROLE-SETTINGS-1调整）

- 项目主控面板现集中在角色列表首项“项目设置”，位于开发要求下方，与具体角色详情互斥显示；列表/详情同步标记并按member_id消歧。human可指定/解除、已有非null指定时候选框及指定按钮置灰；agent只读。失效指定仍显示原因且可解除，不以business_role/模型名推导主控。
- 读取项目三个controller_assignment相关字段，写专用PATCH controller-assignment并携带原版本；不直接覆盖已有指定、不猜版本。未知/旧服务/加载/错误均明确降级，超JS安全整数禁写，409/400重读而不自动重写。assigned仅职责配置，不代表在线、ACK或执行。
- 请求绑定项目/账号/代次，saveToken独立管理保存收尾权，400/409内部GET不导致锁死；离开角色页迟到响应不重绘任务/群聊列表。焦点记忆绑定项目/账号及项目设置上下文，指定成功到解除按钮、解除成功到候选框（不可聚焦时退到状态行），切角色不抢焦。REQ2草稿、单timer与任务群聊原操作保留。
- #112/#114/#116开发与#113/#115/#117复核；117解释器受限partial由Codex补跑填补。Node117、页面契约7及相关34/64/44/6探针通过；旧r114探针唯一版本断言失配如实保留。资源20260919-c1b-s2-owner四处一致；真实页面与窄屏待验收。
- 已知边界和启动/账号/验收步骤见 [主控角色页验收](../guides/CONTROLLER_ASSIGNMENT_ACCEPTANCE.md)。未新增主动模式按钮、多角色创建/复制、进程管理或自动转审。

## 2026-09-23 ROLE-SETTINGS-1：项目设置独立页与角色入口整理

- 角色列表首项固定为“项目设置”，默认首次进入选中，不计角色数量、不作为member_id，搜索/空名册仍可达；具体角色详情与项目设置互斥，项目切换重置选择。开发要求草稿保留原项目/账号隔离与保存归属保护。
- 主控管理集中在项目设置，候选只取名册内已注册未禁用Agent，同名附ID。已有指定（含失效指定）时保留候选行并禁用候选框/指定按钮，明确先解除再另选；human可解除，agent只读。候选集合未变化不重建选项，已有主控的直接覆盖保护不变。
- 移除角色详情“交办任务”按钮及专属绑定，保留查看任务、全局新建任务、共享弹窗、任务树入口与后端/API/MCP权限。相关代码未来保留或清理仍按路线图独立核查。
- #118 Kimi开发、#119 DeepSeek独立复核通过；用户因额度不足明确授权Codex接手余项且不再派发。Codex修正候选隐藏为置灰并同步焦点，复跑Node127、页面契约7通过；这次小改未另做独立Agent复核，#119结果仅覆盖其当时版本。资源20260923-role-settings-1四处一致；用户于2026-09-23确认页面验收通过（eb45298），#118/#119已正式收取。
- 启动和验收见 [主控角色页验收](../guides/CONTROLLER_ASSIGNMENT_ACCEPTANCE.md)。
