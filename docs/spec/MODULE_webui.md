...
## 2026-09-20 C1b-S2：角色页唯一项目主控（待用户页面验收）

- 项目主控面板位于开发要求与角色详情之间，列表/详情同步标记并按member_id消歧。human可指定/解除、已有非null指定时其他角色置灰；agent只读。失效指定仍显示原因且可解除，不以business_role/模型名推导主控。
...
- 请求绑定项目/账号/代次，saveToken独立管理保存收尾权，400/409内部GET不导致锁死；离开角色页迟到响应不重绘任务/群聊列表。焦点记忆含角色，指定/解除成功到下一步可见入口/状态行，切角色不抢焦。REQ2草稿、单timer与任务群聊原操作保留。
