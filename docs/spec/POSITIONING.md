...
- 与 **CCB**（`docs/调研/` 里评估的 agent 协调框架）的关系：
  - **纯任务委派 / RPC（发出去 → 回调结果）**：CCB 的 Mailbox / Callback / Attempt 更轻、更专；TALK 用"聊天室"做纯 RPC 是杀鸡用牛刀，**不该和 CCB 拼这个**。
  - **TALK 不可替代的价值**：Hall 作为**可观测 + 人在环 + 共享上下文**的协作场，尤其是**多 Agent 审议（讨论 / 评审 / 共识）**——这是 CCB 没有的层。
- 判据：**工作越需要"来回交互 / 集体协商"，Hall 越值；越是单向 fire-and-forget，越交给 CCB 式 mailbox。** 两者互补，不冲突。
...
|---|---|---|---|
| **协调类** | 1 单次任务协作、2 任务分配 | 可观测 + 人在环 + 共享上下文 | 机制借 CCB，渲染进 Hall |
...
3. **协调类（1 / 2）借 CCB** —— 投递确认 / 回调 / 重试 / 失败分类，渲染进 Hall。
