...
## 2026-09-18 REQ-1：项目级开发要求（已实现后端/工具）

本节为当前实现，优先于本文历史规划；#92开发、#93独立复核通过。角色页编辑区REQ-2已实现并经#97独立复审通过，真实页面待人工验收。

- `projects.development_requirements` 是可空TEXT，只存最新内容，无版本号或历史；默认NULL。`init_db()` 按列存在性幂等增列，保留老项目与数据。
- 项目创建/详情/列表/更新API新增同名字段，最多20000字符，保留中文及换行；PATCH省略保持原值，显式空白或null清空。读取与写入沿用既有权限：认证成员可读，human可创建/更新；不新增项目隐私或权限合同。
- `cli/talk.py::register_project` 仅在元数据显式提供字段时透传；`TALK/client/`当前无项目helper，新字段经既有项目HTTP API读写。
- `talk_list_agents` 对当前解析项目实时读取并在顶层返回一次要求（含null），无项目时为null。主控派发前读取它与角色清单；不从其它项目或缓存回退。
...
## 2026-09-19 C1b-S：长期项目主控指定（S1后端与S2页面已完成，待页面验收）

本节替代 C1b 会话绑定/租约方案。S1由#110开发、#111独立复核通过；S2页面经#112–#117与Codex补跑验证完成，在线配置与页面人工验收尚未完成。

- 用户确认：在角色页唯一指定项目主控，供Agent读取遵守；保留既有派发/领取/完成/收取权限；指定长期保存，无期限、不续租，直到人工解除或更换。离线与指定分开，不自动唤醒、不宣称正在执行。
- 身份采用项目内具体 `member_id`，不以模型名、runtime、业务角色标签或API供应商凭证作身份。同项目多个Kimi/DeepSeek角色须独立member_id及TALK接入凭证；新建/复制角色和启动实例不在本片。
- C1b-S1先实施后端与MCP只读输出，C1b-S2再实施角色页：至多一个主控，无主控时可指定；已有主控时其他角色的主控选项置灰并提示当前指定，human可主动解除后另选。当前角色业务标签与决策分级保持原义，不因指定自动授予任何宿主权限。
- S1拟用项目级 `controller_member_id`（可空，旧项目默认null）和独立 `controller_assignment_version`（默认0）；不复用C1a模式版本，不自动指定lead/Codex。设置/解除仅human，严格版本条件更新防陈旧页面覆盖；同值且版本正确不增版本，陈旧同值仍冲突。存在其他指定时不能直接覆盖，先显式解除。数据库操作原子保证并发设置至多一个成功。
- 只能新指定已注册、未禁用且在该项目名册中的agent；不限定模型或是否在线。普通项目创建/更新、CLI同步、profile编辑不能旁路写入。名册同步移除或成员禁用不能自动清除长期指定，读取时如实报告当前不可用原因，human仍可解除，恢复名册/启用后可重新正常显示；不自动转给其他成员。
- 项目API及 `talk_list_agents` 只读公开主控指定与版本，并区分未指定/指定有效/指定成员不在名册或已禁用等状态。旧服务不支持时明确不支持，非项目上下文为空；尽量复用已有项目读取，不增加MCP工具。指定有效只表示职责配置有效，不证明在线或会话归属。
- C1a模式意向及既有字段保持兼容，active不自动执行；新增指定字段不能冒充effective_mode已生效。既有任务授权、请求者收取与开发要求快照不变，不自动转审，不新增会话token/ACK/租约表或线程。
- 必要验证：旧库迁移可重复且保留数据；不同项目隔离；human/agent写入权限；候选合法性；并发CAS/陈旧版本/重复同值/清空后再指定；版本严格整数及SQLite边界；名册同步/禁用/重开数据库保留指定；MCP旧服务降级及任务行为回归。所有变更先在隔离库验证，不改真实指定或启停共享服务。
- 后续待办：检查角色固定规范、派发时验收标准、交付/差异自动组装与自动转审；需求变化或分歧返回主控。当前仍由Codex转交审查，不在本片自动化。


### S1实际接口与验收（2026-09-19）

- `PATCH /api/projects/{project_id}/controller-assignment` 仅human：请求必填 `member_id`（去首尾空白后的非空字符串或显式null）及 `expected_version`（严格整数0..2**63-1）；缺键/空串/非法类型/版本越界422。返回ProjectOut。解除用null，已有不同指定时409，须先解除再另选。
- 正确版本同值200且不写/不增版本；陈旧同值409；实际改变通过条件UPDATE原子+1，版本耗尽时409不溢出。候选非法400（未注册、非agent、禁用、不在名册），失败不写。未认证/agent写入沿用既有鉴权语义。普通项目入口不能旁路写指定与版本。
- 项目读取字段：`controller_member_id`、`controller_assignment_version`、`controller_assignment_status`；状态为unassigned/assigned/not_in_roster/member_disabled/member_missing/not_agent，名册/禁用变化不自动清除指定。assigned仅表示当前职责配置有效，不证明在线或实际执行。
- `talk_list_agents.controller_assignment` 为supported/member_id/version/status/note对象，复用同一次项目GET；旧服务unsupported，无项目null，未知状态unknown；仍9个工具，C1a模式及开发要求兼容。
- 已披露且本片接受的限制：候选校验至CAS写入之间若发生并发sync移出/禁用，指定可能保存成功，随后的读取如实报not_in_roster/member_disabled。唯一性与版本CAS仍成立；这不是候选有效性在提交瞬间的原子保证。因指定不附带权限、当前无强制派发门禁，本片不扩大锁改造；未来可加强事务原子性。S2须展示无效原因并允许human解除，不能把保存成功等同于有效/在线。
- #111独立复跑新增44项、定向54项、spawn相关169项及不排除用例的全量716项全部通过，计数包含重叠不相加；补齐#110的24项沙箱限制。独立竞态探针验证上述窗口。仅单进程多线程SQLite WAL并发，跨进程未验证。
- #110引用的一份44项明细文件缺失，由#111独立复跑证据补足；#111服务器结果带引言/围栏为unknown，本地review.json实际任务号111校验有效。运行中断及用户重启与功能复核分开记录，不证明真实库完整性或未提交代码未加载。共享服务/MCP加载与真实页面验证另行完成。

- 2026-09-20 S2页面已接入本接口，human指定/解除、失效指定解除、其他角色置灰、agent只读和请求隔离已验证；真实页面验收见 `docs/guides/CONTROLLER_ASSIGNMENT_ACCEPTANCE.md`，不将前端完成当作在线主控已指定。
