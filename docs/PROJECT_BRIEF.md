...
用户最新确认优先：C1b改为角色页唯一指定项目主控、职责长期保存直到人工解除或更换，不采用会话租约/续租，保留现有任务权限。C1b-S1后端与MCP读取已由#110/#111开发/独立复核通过（716项全量测试），S2页面待做；旧会话合同仅历史归档。自动转审与多角色创建入口后续，见PROJECT_INTEGRATION最新节。
