...
桌面Web容器与服务生命周期、工作区角色会话隔离、多终端主控能力验证、主被动按钮及任务命名的统一排期见 [DEVELOPMENT_ROADMAP.md](spec/DEVELOPMENT_ROADMAP.md)。保留原生Agent运行器，不将TALK重建为模型执行框架；N1 统一任务标题已完成开发与独立复核，页面待人工验收；L1-1 Kimi 独立入口准备已完成开发复核，真实模型主控闭环未运行。新增角色列表项目级开发要求（仅最新内容、无版本历史）已记录待开发；后续按路线逐片推进。
