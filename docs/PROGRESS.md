...
- C1b-0设计已完成独立复核并收尾：#104设计→#105发现问题→#106修订→#107剩余R1→#108定向修订→#109通过。Codex确认R1/R1-a/R1-b关闭，#104–#109于2026-09-19正式收取（结果msg2599–2604）；保留#105/#107历史blocked和#105服务器unknown/本地有效报告的差异。已复核v3归档 `docs/spec/C1B_CONTROLLER_CONTRACT_REVIEWED.md`，基线 `c20640b`。D1绑定权限、D2任务兼容、D3持久化/过期策略待用户确认，尚未启动C1b-1。只完成设计，无业务代码/数据库/API改动；细节与验证边界见历史记录。
...
1. 等用户确认D1/D2/D3：推荐项目名册内agent仅在无活跃归属时绑定，human可经强制释放接管且遵守并发检查；既有任务入口不加主控门禁；绑定持久化、按租约过期。确认后另派C1b-1实现，C2按钮后续。
