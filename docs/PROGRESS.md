...
## Latest
Updated: 2026-06-19 (Asia/Shanghai) — Phase 2 server 端完整（切片 7–9）；codex Hall 验收通过；pi 修复 pi.CMD 换行截断 bug（待 Hall 复验）；CCB 调研已登记
...
- **人工验收（2026-06-19，已带 `--project` 真实跑 `test-run20` / `group:843d8433bae1`）**：
  ...
  - pi 8b：**先发现真问题再修复**——`@agent:pi` 只回 bridge 兜底句"我是 pi，已切换为中文回复…"。逐步黑盒定位（与 8b 注入/plan-mode/工具路由均无关）：**pi 0.79.8 经 Windows `pi.CMD` shim 时，`--system-prompt` 含换行就被 cmd.exe 截断 → pi 完全不产出 → bridge 中文兜底**。修复：`pi_bridge._single_line()` 把系统提示（含注入 profile）压成一行（commit `5b57042`）。修后真机：pi 默认/注入命令均恢复出中文，且注入命令真正体现 agent:pi 人设。**注意需重启 pi bridge 后在 Hall 复验。**
