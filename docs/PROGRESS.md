...
## Latest
Updated: 2026-06-20 (Asia/Shanghai) — Phase 1+2 已合入 `main`（PR #1）；分支 `claude/phase3-collab-and-ui`（已 push）。本轮：P3-1 + P3-2 + UI #2 删 Hall（全栈）+ UI #3 全局禁用 agent（全栈，前端开关待真机）+ 数据清理（群 31→1、成员→5）。下一片/独立子阶段 = P3-3 MEMORY（完整 server 端）。Claude=决策 Agent（管理者授权自主开发）
...
- **切片 UI #3 全局禁用 agent 后端完成**：`Member` 加 `disabled_at`（软删，保留行 + `messages.from_id` 归属 + 群成员关系，不自动退群）；`get_current_member` 对已禁用成员返回 403（拒鉴权）；`PATCH /api/members/{id}`（仅人类、仅 agent 目标）切换启用/禁用；`MemberOut` 加 `disabled_at`；`db.py` 幂等列迁移 + 索引。+4 单测（禁用→403→重启用 / agent 拒操作 / 不可禁用 human / 缺成员 404）；鉴权子集 69 测无回归。
- **切片 UI #3 前端完成**：右侧"全部 agent 列表"改为**只列 agent**（不再展示 human）；每个 agent 行加"禁用/启用"开关（仅人类可见）→ `PATCH /api/members/{id}`；已禁用行名称置灰 + "已禁用"徽标、"加入"按钮禁用（不可把禁用 agent 拉进群）。新增 `toggleMemberDisabled` + `.member-toggle-btn`/`.member-disabled-badge` 样式。**管理者真机功能验收通过**；按反馈做了排版收尾：列表已 agent-only，移除每行冗余 `agent` 标签 + 过时"点角色筛选"提示、标题改"所有 Agent"、按钮加 `white-space:nowrap`，解决名字被挤成单字 / "已在 Hall"换行的拥挤问题（版本号 `20260620-agent-disable-2`）。
- **数据清理（管理者授权，2026-06-20）**：① API 删 30 个老测试群，仅留 `test-run20`（群 31→1）；② 直接删 DB 清掉 5 个测试成员（agent：ui52226/testpi/pi@projA:tester；human：tester/ui52226），仅 0 消息者才删以保归属；现存 5 成员 = agent `codex`/`pi`/`pi-kimi` + human `bobo`/`qa`。
- **本分支切片路线**：P3-1 ✓ → P3-2 ✓ → UI #2 删 Hall（全栈）✓ → UI #3 全局禁用 agent（全栈）✓ → **P3-3 MEMORY（下一片/独立子阶段：完整 server 端 COLD/WARM/RESUME，范围大）**。
