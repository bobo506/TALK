...
- DSH真实模型主控尚未运行：缺少可用于试验的合法本人凭证来源，真实模型工具可见、委派与同会话收取仍待验证。原生ACP零模型恢复已通过；headless仍不支持原会话恢复。测试残留一条模拟进程管道ResourceWarning，不阻塞本片，后续单独处理。
- 既有DSH入口准备已推送 `3fcbf44`。本片进入提交收尾；不将任务completed等同DSH真实主控验收。用户通知后再取件，不主动等待。
...
1. 按用户最新确认继续终端验证：下一片在合法本人凭证可用后验证 DSH 真实模型工具可见、委派及 ACP 同会话收取，再 WorkBuddy；逐终端输出配置即可/需适配/暂不支持的实测结论。完整纳入页面启停与工作区会话管理仍需后续运行器适配开发，可通过TALK版本升级支持新终端。
