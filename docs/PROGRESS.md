...
- DSH 入口准备已完成：#70 开发、#71 复核、#72 返工、#73 再复核，四项已收取；基线 `9423d02`。新增无密钥覆盖层、启动器、config/check/probe/dump 预检与18项回归。Kimi独立复跑61项组合测试及真实DSH中文/空格相对路径dump通过；修正了路径解析、期望项目核对与异常收口。
- DSH 0.1.5-rc.1 真实模型主控尚未运行：执行者嵌套shell过滤KEY环境变量、本人外部密钥文件未配置；headless不支持原会话恢复，sdk-jsonrpc也不是持久恢复路径，ACP session/resume为待验证候选。普通受控启动不要求默认放宽沙箱，真实凭证传递/工具可见/委派/同会话收取仍未实测。
- 只收取DSH入口工具成果，不将流程completed等同DSH主控验收。既有Kimi单任务闭环已推送 `9423d02`。当前恢复被动模式，用户通知后再取件，不主动等待。
...
1. 按用户最新确认继续终端验证：下一片完成 DSH 本人凭证安全传递与 ACP 原生同会话方案验证，再 WorkBuddy；逐终端输出配置即可/需适配/暂不支持的实测结论。完整纳入页面启停与工作区会话管理仍需后续运行器适配开发，可通过TALK版本升级支持新终端。
