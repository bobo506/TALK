...
- **切片 UI #3 前端完成**：右侧"全部 agent 列表"改为**只列 agent**（不再展示 human）；每个 agent 行加"禁用/启用"开关（仅人类可见）→ `PATCH /api/members/{id}`；已禁用行名称置灰 + "已禁用"徽标、"加入"按钮禁用（不可把禁用 agent 拉进群）。新增 `toggleMemberDisabled` + `.member-toggle-btn`/`.member-disabled-badge` 样式；版本号 `20260620-agent-disable`。静态校验通过 + 运行中 server 实测服务新文件。**开关点选端到端待管理者**——需重启 server 让 UI #3 后端 `PATCH` 端点上线。
