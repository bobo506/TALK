...
- 现有 `.talk/groups.yaml` 中 `agent:pi` / `agent:pi-kimi` 仍是 reviewer 配置，没有正式的全能力 Tester profile。
...
1. 项目管理者人工验收 TH-6d：重点检查里程碑 Test 通过后的自动暂停、门禁摘要、人工验收按钮以及返工后旧结论失效。
2. 验收通过后进入 TH-7：补 Codex Desktop / 通用终端接入包装并做完整跨终端验收。
3. 后续再处理正式 Tester profile、附件正文注入、预检退避和跨实例消息级去重。
