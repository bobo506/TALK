#!/usr/bin/env python3
"""Kimi Code 独立会话的 TALK MCP 启动器（仓库内不存放任何密钥）。

Kimi Code CLI 按官方 `mcp.json` 的 stdio 条目启动本脚本；本脚本只做两件事：
...
DEFAULT_KEY_FILE_NAME = "agent-kimi.key"
DEFAULT_KEY_DIR_NAME = ".talk"


class KimiEntryError(RuntimeError):
    """启动器可预期的中文错误；调用方只打印 message，不打印 traceback。"""


def default_key_file() -> Path:
    """仓库外的默认密钥文件位置：``~/.talk/agent-kimi.key``。"""
    home = os.environ.get("USERPROFILE") or os.environ.get("HOME")
    if not home:
        try:
            home = str(Path.home())
        except RuntimeError as exc:
            raise KimiEntryError(
                "无法确定用户主目录；请用 TALK_KIMI_KEY_FILE 显式指定仓库外的密钥文件"
            ) from exc
...
def _read_key_file(path: Path, *, repo_root: Path) -> str:
    ...
    try:
        # utf-8-sig：容忍 Windows PowerShell 5.1 `Set-Content -Encoding utf8` 写入的 BOM。
        text = resolved.read_text(encoding="utf-8-sig")
    except (OSError, UnicodeError) as exc:
        raise KimiEntryError(f"读取密钥文件失败（{resolved}）：{exc}") from exc
    for line in text.splitlines():
        candidate = line.strip()
        if candidate:
            return _validate_key(candidate, source=f"密钥文件 {resolved}")
    raise KimiEntryError(f"密钥文件为空：{resolved}")
...
def resolve_api_key(
    *,
    ...
    key_file = (env.get("TALK_KIMI_KEY_FILE") or "").strip()
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Kimi Code 独立会话的 TALK MCP 启动器（复用 bridges/talk_terminal_mcp.py）",
    )
    parser.add_argument(
        "--project-root",
        type=Path,
...
def main(argv: list[str] | None = None) -> int:
    ...
    except KimiEntryError as exc:
        print(f"Kimi TALK 入口启动失败：{exc}", file=sys.stderr)
        return 1
    ...
    try:
        ...
        print(f"Kimi TALK 入口启动失败：无法导入 TALK 终端入口（{exc}）", file=sys.stderr)
        return 1

    print(f"Kimi TALK 入口：密钥来源 {source}；工作区 {project_root}", file=sys.stderr)
    forwarded = ["--project-root", str(project_root), *passthrough]
