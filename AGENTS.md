...
- 2026-05-14 起，Codex 在本项目后续协作中被授权作为**决策 Agent**：可在需求方向已明确或需要保持项目连续性时，主动更新项目文档、模块文档、进度记录与必要的决策说明
- 开发过程中只要存在任何不确定项，都必须先向项目管理者确认，再继续开发
- “不确定项”包括：需求理解不明确、实现路径有多种可选方案、Agent 认为存在更优替代方案、可能影响现有行为或接口的改动
- 对未获授权的普通开发执行 Agent：除 `docs/PROGRESS.md` 外，其它项目文档默认不得擅自修改；如需改动，必须先得到项目管理者明确许可
- 对已授权的决策 Agent：可以直接维护与当前工作相关的项目文档，但应保持改动聚焦、可追溯，不顺手扩写无关内容
- 一旦项目管理者明确确认某项需求/实现决策，并允许同步文档，Agent 应在相关代码落地后及时更新对应文档，使文档与当前实现保持一致
- 文档更新范围应限制在与本次已确认改动直接相关的模块文档和公共简报，不得顺手扩写无关内容
- `docs/PROGRESS.md` 是本项目唯一进度文档；开发执行 Agent 完成实际代码、测试或文档工作后，可以直接更新该文件，记录完成内容、验证结果、变更文件、待确认问题和下一步计划
